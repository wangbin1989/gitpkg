...
### Fixed
- 补全脚本添加 `link` 命令的动态补全支持（已安装工具名称）

## [2.8.0] - 2026-08-15

### Added
- 新增 `link` 命令，为已安装工具重新创建符号链接（适用于链接丢失或损坏的场景）
...
### Fixed
- `outdated` 命令遇到 GitHub API 限流时不再误报"所有工具均为最新版本"，改为显示真实错误提示
- `info` 命令不再将限流等 API 错误静默显示为"（无 Release）"，仅在仓库确实无 Release 时显示

## [2.5.4] - 2026-07-22

### Fixed
- 修复 zsh/bash 自动补全无法解析格式化 JSON（带缩进）的问题，grep 模式兼容有无空格两种格式
